...
export const HomeTab: React.FC<{ state: AppState, onToggleLike?: () => void, setActiveTab?: (tab: TabType) => void }> = ({ state, onToggleLike, setActiveTab }) => {
...
export const AIAssistantTab: React.FC<{ 
  ...
  const isPlayingMusic = state.isPlayingMusic;
  const setIsPlayingMusic = (playing: boolean) => onUpdateState?.({ isPlayingMusic: playing });
  ...
  const handleSendMessage = async (text?: string, image?: string) => {
    ...
    // Resume audio context immediately on user gesture for mobile support
    if (!audioContextRef.current) {
      audioContextRef.current = new (window.AudioContext || (window as any).webkitAudioContext)();
    }
    if (audioContextRef.current.state === 'suspended') {
    ...
    try {
      ...
      setIsThinking(false);
      const errorMsg = state.language === 'English' 
        ? "Connection error. Please check your network or API key." 
        : "连接似乎出了点问题，请检查网络或 API Key 设置。";
      setMessages(prev => [...prev, { id: (Date.now() + 2).toString(), role: 'ai', content: errorMsg }]);
    }
  };
